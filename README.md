...
```bash
git clone https://github.com/your-org/namgun-workspace.git
cd namgun-workspace
sudo bash setup.sh
```
...
| 버전 | 날짜 | 내용 |
|------|------|------|
| v4.1.0 | 2026-03-09 | Phase 10: PWA + 모바일 — @vite-pwa/nuxt 통합, Web App Manifest (standalone), Service Worker (Workbox 캐싱), 오프라인 fallback, 앱 설치 프롬프트, 푸시 알림 기반 (VAPID+pywebpush), iOS/Android/Windows 메타 태그, usePwa 컴포저블 |
| v4.0.2 | 2026-03-09 | Phase 9 완료 — `update.sh` 무중단 업데이트 (순차 재빌드+헬스체크+롤백), `health.sh` 서비스 자가진단 (Docker/DB/Redis/API/디스크/NFS/SSL, --json 모니터링 연동), ROADMAP Phase 6~9 상태 갱신 |
