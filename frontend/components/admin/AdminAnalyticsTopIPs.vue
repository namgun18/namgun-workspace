...
import type { TopIP } from '~/composables/useAdminAnalytics'

ChartJS.register(CategoryScale, LinearScale, BarElement, Tooltip)

const props = defineProps<{ data: TopIP[] }>()

const colorMode = useColorMode()

const chartData = computed(() => ({
  labels: props.data.map(d => d.ip_address),
  datasets: [{
...
<template>
  <div class="rounded-lg border bg-card p-4">
    <h3 class="text-sm font-medium text-muted-foreground mb-3">{{ $t('admin.analytics.topIPs.title') }}</h3>
    <div class="h-[240px]">
