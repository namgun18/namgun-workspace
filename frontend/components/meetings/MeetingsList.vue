...
<template>
  <div class="space-y-4">
    <div class="flex items-center justify-between">
      <h2 class="text-xl font-semibold">{{ $t('meetings.list.title') }}</h2>
      <button
        @click="emit('create')"
    ...
    <div v-else-if="rooms.length === 0" class="text-center py-12">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="h-12 w-12 mx-auto text-muted-foreground/50 mb-3" aria-hidden="true">
        <path d="m22 8-6 4 6 4V8Z" /><rect width="14" height="12" x="1" y="6" rx="2" ry="2" />
      </svg>
      <p class="text-muted-foreground">{{ $t('meetings.list.empty') }}</p>
      <p class="text-sm text-muted-foreground/70 mt-1">{{ $t('meetings.list.emptyHint') }}</p>
    </div>
    ...
    <div v-else class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      ...
      >
        <div class="flex items-start justify-between mb-3">
          <div>
            <div class="flex items-center gap-2">
              ...
              <span v-if="room.is_host" class="text-[10px] font-medium px-1.5 py-0.5 rounded bg-primary/10 text-primary">{{ $t('meetings.list.host') }}</span>
            </div>
          ...
          <div class="flex flex-col items-end gap-1">
            ...
            <span v-if="room.pending_count > 0" class="text-[10px] px-1.5 py-0.5 rounded-full bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400">
              {{ $t('meetings.list.pending', { n: room.pending_count }) }}
            </span>
          </div>
        </div>
        ...
        <div class="flex gap-2">
          ...
          >
            {{ copiedToken === room.share_token ? $t('common.copied') : $t('meetings.list.copyLink') }}
          </button>
          <button
