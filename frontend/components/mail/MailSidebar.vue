...
function isProtected(mb: any) {
  return mb.role && PROTECTED_ROLES.has(mb.role)
}
...
function toggleMenu(mbId: string, e: Event) {
  e.stopPropagation()
  openMenu.value = openMenu.value === mbId ? null : mbId
}
  ...
  <aside class="flex flex-col h-full border-r bg-muted/30">
    ...
    <nav class="flex-1 p-3 space-y-0.5 overflow-auto">
      <template v-for="mb in mailboxes" :key="mb.id">
        ...
        <div v-else class="relative group flex items-center">
          ...
          <!-- More menu button (custom folders only) -->
          <div v-if="!isProtected(mb)" class="relative">
            <button
              @click="toggleMenu(mb.id, $event)"
              class="h-6 w-6 flex items-center justify-center rounded hover:bg-accent text-muted-foreground hover:text-foreground transition-colors opacity-0 group-hover:opacity-100"
              :class="{ 'opacity-100': openMenu === mb.id }"
            >
            ...
            <div
              v-if="openMenu === mb.id"
              class="absolute right-0 top-7 z-50 bg-popover border rounded-md shadow-md py-1 min-w-[120px]"
              @click.stop
            >
              <button
                @click="startRename(mb)"
                class="w-full px-3 py-1.5 text-sm text-left hover:bg-accent transition-colors"
              >{{ $t('mail.folder.rename') }}</button>
              <button
                @click="handleDeleteMailbox(mb)"
    ...
    <div class="px-3 py-2 border-t">
      <button
        ...
        class="w-full flex items-center gap-2 px-3 py-2 text-sm rounded-md text-muted-foreground hover:bg-accent/50 hover:text-foreground transition-colors"
      >
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4">
    ...
    <!-- Filter rules dialog -->
    <MailFilters v-if="showFilters" @close="showFilters = false" />
  </aside>
</template>
