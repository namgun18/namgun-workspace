...
async function handleDeleteSelected() {
  ...
  if (!confirm(t('files.commandBar.deleteConfirm', { n: selectedItems.value.size }))) return
  ...
  <div class="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-4 py-2 border-b bg-background">
    ...
    <div class="relative">
      ...
      >
        ...
        <span class="hidden xs:inline">{{ $t('files.commandBar.new') }}</span>
    ...
    >
      ...
      <span class="hidden sm:inline">{{ $t('files.commandBar.upload') }}</span>
    </button>
    ...
    <template v-if="selectedItems.size > 0">
      ...
      <span class="text-xs text-muted-foreground hidden sm:inline">{{ $t('files.commandBar.selectedCount', { n: selectedItems.size }) }}</span>
    </template>
    ...
    <button
      ...
      :title="viewMode === 'list' ? $t('files.commandBar.gridView') : $t('files.commandBar.listView')"
    >
