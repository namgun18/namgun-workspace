...
<template>
  <div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50" @click.self="emit('close')">
    <div class="bg-background rounded-t-xl sm:rounded-lg shadow-xl w-full sm:max-w-lg sm:mx-4" role="dialog">
      <!-- Header -->
      <div class="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b">
        <h2 class="text-base sm:text-lg font-semibold">{{ $t('files.upload.title') }}</h2>
      ...
      <div class="p-4 sm:p-6">
        <p class="text-sm text-muted-foreground mb-3 sm:mb-4">
          {{ $t('files.upload.destination') }} <span class="font-medium text-foreground">{{ currentPath || '/' }}</span>
        </p>
        ...
        >
          ...
          <p v-if="isDragging" class="text-sm text-primary font-medium">{{ $t('files.upload.dropHere') }}</p>
          <template v-else>
            <p class="text-sm text-muted-foreground">{{ $t('files.upload.dragOrClick') }}</p>
            <p class="text-xs text-muted-foreground/70 mt-1">{{ $t('files.upload.maxSize') }}</p>
          </template>
        </div>
        ...
        <div v-if="uploading" class="mt-4 text-center">
          ...
          <p class="text-sm text-muted-foreground mt-2">{{ $t('files.upload.uploading') }}</p>
        </div>
