...
async function handleCreate() {
  ...
  try {
    ...
    alert(e?.data?.detail || t('files.share.createError'))
  } finally {
...
<template>
  <div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50" @click.self="emit('close')">
    <div class="bg-background rounded-t-xl sm:rounded-lg shadow-xl w-full sm:max-w-md sm:mx-4" role="dialog">
      <!-- Header -->
      <div class="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b">
        <h2 class="text-base sm:text-lg font-semibold">{{ $t('files.share.title') }}</h2>
      ...
      <div class="p-4 sm:p-6 space-y-4">
        <div>
          <p class="text-sm text-muted-foreground">{{ $t('files.share.fileLabel') }}</p>
          <p class="text-sm font-medium">{{ item.name }}</p>
        </div>

        <!-- Generated URL -->
        <div v-if="shareUrl">
          <label class="text-sm text-muted-foreground block mb-1">{{ $t('files.share.linkLabel') }}</label>
        ...
        <template v-else>
          <div>
            <label class="text-sm text-muted-foreground block mb-1">{{ $t('files.share.expiresLabel') }}</label>
            <select v-model="expiresIn" class="w-full px-3 py-2 text-sm bg-background border rounded-md focus:outline-none focus:ring-1 focus:ring-primary">
              <option value="">{{ $t('files.share.noExpiry') }}</option>
              <option value="1h">{{ $t('files.share.expire1h') }}</option>
              <option value="1d">{{ $t('files.share.expire1d') }}</option>
              <option value="7d">{{ $t('files.share.expire7d') }}</option>
            </select>
          </div>

          <label class="flex items-center gap-2 text-sm cursor-pointer">
            <input v-model="oneTime" type="checkbox" class="rounded border-muted-foreground/30" />
            {{ $t('files.share.oneTime') }}
          </label>
        </template>
      </div>
      ...
      <div class="flex justify-end gap-2 px-4 sm:px-6 py-3 border-t">
        ...
        >
          {{ creating ? $t('common.creating') : $t('files.share.createLink') }}
        </button>
      </div>
